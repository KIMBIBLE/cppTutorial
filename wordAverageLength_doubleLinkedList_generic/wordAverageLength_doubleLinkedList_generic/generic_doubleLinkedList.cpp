...
{
	lp->head = (Node *)calloc(1, sizeof(Node) + size); //헤드 노드 생성
	assert(lp->head != NULL);
	lp->tail = (Node *)calloc(1, sizeof(Node) + size); //테일 노드 생성
	assert(lp->tail != NULL);
...
{
	Node *np;
	np = (Node *)calloc(1, sizeof(Node) + size); //새로운 노드 생성
	assert(np != NULL);
	np->prev = prev;
	np->next = next;

	//myMemCpy((char*)np+8, dataPtr) 같은표현
...
{
	lp->cur = makeNode(dataPtr, lp->head, lp->head->next, sizeof(Node) + size, myMemCpy);  //새로운 노드 생성
	assert(lp->cur != NULL);
...
{
	lp->cur = makeNode(dataPtr, lp->tail->prev, lp->tail, sizeof(Node) + size, myMemCpy);  //새로운 노드 생성
	assert(lp->cur != NULL);
	lp->length++; //데이터개수 1 증가
...
리턴값: 없음
--------------------------------------------------------------------------------------*/
void display(LinkedList *lp, void(*print)(void *, int num))
{
	int num = 1;
	lp->cur = lp->head->next;
...
Node * searchUnique(LinkedList *lp, void *dataPtr, int(*compare)(void *, void *))
{
	...
	{
		if (compare(lp->cur + 1, dataPtr) == 0) //데이터가 있으면 현재 위치 리턴
		{
			return lp->cur;
		}
...
Node ** searchDuplicate(LinkedList *lp, int *searchCnt, void *dataPtr, int(*compare)(void *, void *)) // dataPtr과 일치하는 노드의 주소를 담고 있는 Node *배열의 시작주소 리턴/없을 시 NULL pointer 리턴
{
	...
	*searchCnt = 0; // 찾은 노드의 개수를 count할 변수를 0으로 초기화
	...
	while (lp->cur != lp->tail) // 맨 뒷 노드까지 검색
	{
		if (compare(lp->cur + 1, dataPtr) == 0) // 찾았으면
		{
...
void destroy(LinkedList *lp, void(*dataFree)(void *))
{
	...
	{
		deleteNode(lp, lp->head->next, dataFree); // head 노드 바로 뒷 노드를 삭제
	}
	dataFree(lp->head + 1);
...
void sortList(LinkedList *lp, int(*compare)(void *, void *), void(*myMemCpy)(void *, void *), void(*dataFree)(void *))  // 노드 정렬
{
	...
	temp.wp = (char *)malloc(sizeof(char) * 20);
	assert(temp.wp != NULL);
