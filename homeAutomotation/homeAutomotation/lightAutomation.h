...
public:
	...
	int getLightCnt() const;       // lightCnt 멤버 값 반환 함수
							 ...
							 // member function
	bool addAppliance(LightFixture &ap);	      // parameter의 전등객체를 목록에 추가. 동일한 이름의 전등객체는 등록되지 않도록 함
	int searchMachine(string machineName);    // 전등명에 해당하는 전등을 찾아서 배열의 index리턴/없을 시 -1 리턴
	bool deleteAppliance(string machineName);  // 전등명에 해당하는 전등을 목록에서 삭제하다
	void listDisplayAppliance();               // 등록된 전등 전체 목록 및 On.Off 상태를 확인(출력)하다
	bool controlAppliance(string machineName); // 해당 전등의 상태를 변경한다. (On->Off, Off->On 으로 변경)
private:
